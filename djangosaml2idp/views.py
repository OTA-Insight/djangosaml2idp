...
class IdPHandlerViewMixin:
    ...
    def render_response(self, request, html_response):
        ...
        already_agreed = AgreementRecord.objects.filter(user=request.user, sp_entity_id=self.sp['id'])
...
class UserAgreementScreen(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get('dont_show_again') == "Yes":
            record = AgreementRecord(request.user, request.session['sp_entity_id'])
            record.save()
